...
module.exports.UpdateTimesheet = async (req, res) => {
    try{
        ...
        const getTimesheet = await Timesheet.find({$and:[{workDate: FullDate,userId: req.user.user_id}]});
            const updateStartToday = (getTimesheet[0]['checkin']).split(':');
            const updateEndToday = (getTimesheet[0]['checkout']).split(':');
            const updateStartCaltoSec = (+updateStartToday[0]) * 60 * 60 + (+updateStartToday[1]) * 60 + (+updateStartToday[2]);
            const updateEndCaltoSec = (+updateEndToday[0]) * 60 * 60 + (+updateEndToday[1]) * 60 + (+updateEndToday[2]);
            const Minus_Sec = updateEndCaltoSec - updateStartCaltoSec ;
            const chk_CalDate = ((Minus_Sec/60)/60).toFixed(2);
            const TotalTime = await Timesheet.updateOne({name: req.user.name, workDate: FullDate}, {$set:{total: chk_CalDate}})
            const reChk_checkout = await Timesheet.find({$and:[{workDate: FullDate,userId: req.user.user_id, name: req.user.name}]});
        ...
        return res.status(200).send({message: "Get Timesheet from date Success"});
    }catch(error){
